...
const mergeWithDefaults = (apiData: unknown, defaultData: ComoFazemosData): ComoFazemosData => {
  ...
  const sections: (keyof ComoFazemosData)[] = ['ecommerce', 'marketing', 'sobre'];
  const result: Partial<ComoFazemosData> = {};
  
  sections.forEach(section => {
    if (data[section]) {
      const sectionData = data[section] as any;
      const header = { ...defaultData[section].header, ...sectionData.header };
      
      let services = sectionData.services || [];
      if (services.length === 0 && section === 'ecommerce') {
        services = defaultData.ecommerce.services;
      }
      ...
      if (section === 'marketing') {
         stats = sectionData.stats || defaultMarketingData.stats;
         const apiFlywheel = sectionData.flywheel;
...
interface ComoFazemosProps {
  initialActiveSection?: keyof ComoFazemosData;
}

export default function ComoFazemosComponent({ initialActiveSection = 'marketing' }: ComoFazemosProps) {
  const {
    data: pageData,
    exists,
    loading,
  ...
  const [expandedSections, setExpandedSections] = useState({
    ecommerce: initialActiveSection === 'ecommerce',
  ...
  const updateFlywheel = (path: string, value: unknown) => {
  ...
  const calculateCompletion = () => {
    ...
    sections.forEach(section => {
      ...
      if (section !== 'ecommerce' && sectionData.header.gradientTitle?.trim()) completed++;
        ...
        {isMarketing ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="space-y-4">
               ...
               <div>
                  <label className="block text-sm font-medium text-[var(--color-secondary)] mb-2">Título Principal</label>
                  <Input type="text" value={header.title || ""} onChange={(e) => handleHeaderChange(section, "title", e.target.value)} placeholder="Ex: Acelere seu" className="bg-[var(--color-background-body)] border-[var(--color-border)] text-[var(--color-secondary)]" />
               </div>
               <div>
             ...
             <div className="space-y-4">
               <div>
                  ...
                  <TextArea value={header.description || ""} onChange={(e) => handleHeaderChange(section, "description", e.target.value)} rows={5} className="bg-[var(--color-background-body)] border-[var(--color-border)] text-[var(--color-secondary)]" />
               </div>
             </div>
          </div>
        ) : (
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                  ...
                                  <select
                                    ...
                                    className="flex h-10 w-full rounded-md border border-[var(--color-border)] bg-[var(--color-background-body)] px-3 py-2 text-sm text-[var(--color-secondary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] disabled:cursor-not-allowed disabled:opacity-50"
                                  >
      ...
      <div className="space-y-4">
        ...
        <motion.div initial={false} animate={{ height: isExpanded ? "auto" : 0 }} className="overflow-hidden">
          <div className="space-y-6 mt-4">
            ...
            {section !== 'marketing' && renderServicesSection(section, sectionTitle, Icon)}
  ...
  return (
    <ManageLayout headerIcon={Layout} title="Como Fazemos" description="Configure as seções de metodologia e serviços" exists={!!exists} itemName="Seção">
      <form onSubmit={handleSubmit} className="space-y-6 pb-32">
        {renderSection("ecommerce", "Ecommerce", Layout)}
      ...
      <DeleteConfirmationModal isOpen={deleteModal.isOpen} onClose={closeDeleteModal} onConfirm={confirmDelete} type={deleteModal.type} itemTitle={deleteModal.title} totalItems={3} itemName="Configuração" />
