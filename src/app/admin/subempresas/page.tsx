...
import { Network, Plus, Edit2, Trash2, Loader2, X, Upload } from "lucide-react";
...
export default function SubEmpresasCRUD() {
  ...
  // Estado incluindo os campos de JSON como strings
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [subRes, compRes] = await Promise.all([ fetch("/api/sub-companies"), fetch("/api/companies") ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. VALIDAÇÃO DE JSON NO FRONTEND
    if (formData.theme && formData.theme.trim() !== "") {
      try {
        JSON.parse(formData.theme);
      } catch (err) {
        alert("⚠️ O JSON do Tema possui erros de sintaxe. Verifique as aspas e vírgulas.");
        return;
      }
    }

    if (formData.menuItems && formData.menuItems.trim() !== "") {
      try {
        JSON.parse(formData.menuItems);
      } catch (err) {
        alert("⚠️ O JSON do Menu possui erros de sintaxe. Verifique as aspas e vírgulas.");
        return;
      }
    }
    ...
    try {
      ...
      // Envia os JSONs validados como string
  ...
  const openEdit = (sub: any) => {
    // Função para pegar o Objeto/Array do banco e transformar de volta em texto formatado para o textarea
    ...
    setFormData({ 
      ...
      ga_id: sub.ga_id || "", 
      logo_img: sub.logo_img || null,
      theme: formatJson(sub.theme), 
      menuItems: formatJson(sub.menuItems)
    });
              ...
              <table className="w-full text-left text-sm text-zinc-300">
                <thead className="bg-zinc-950/50 text-zinc-400 text-xs uppercase font-semibold">
                  <tr>
                    ...
                    <th className="px-6 py-4">Tracking (GA)</th>
                ...
                <tbody className="divide-y divide-zinc-800/50">
                  {subCompanies.map(sub => (
                    <tr key={sub.id} className="hover:bg-zinc-800/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-zinc-500">{sub.ga_id || "Não configurado"}</td>
        ...
        <AnimatePresence>
          {isModalOpen && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-xl bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-zinc-400 uppercase mb-2">Estrutura do Menu (JSON)</label>
                    <textarea 
                      value={formData.menuItems} 
                      onChange={(e) => setFormData({...formData, menuItems: e.target.value})} 
                      className="w-full p-3 bg-black/50 border border-zinc-800 rounded-lg text-indigo-400 font-mono text-xs outline-none focus:border-cyan-500 resize-none h-40 custom-scrollbar" 
                      placeholder='[\n  {\n    "type": "group",\n    "title": "Home",\n    "icon": "FileText"\n  }\n]' 
                    />
                  </div>
